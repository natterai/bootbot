...
class BootBot extends EventEmitter {
  constructor(options, logger) {
    ...
    if (!options || (options && (!options.accessToken || !options.verifyToken))) {
      throw new Error('You need to specify an accessToken and verifyToken');
    }
    if (!logger) {
    ...
    this.graphUrl = options.graphUrl || 'https://graph.facebook.com/v2.6';
  ...
  start(port) {
    ...
    this.server = this.app.listen(this.app.get('port'), () => {
      ...
      this.logger.info('BootBot running on port', portNum);
      this.logger.info(`Facebook Webhook running on localhost:${portNum}${this.webhook}`);
    });
  }
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    return fetch(`${this.graphUrl}/me/${endpoint}?access_token=${this.accessToken}`, {
      method,
      headers: {
    ...
    .then(res => {
      if (res.error) {
        this.logger.error('Messenger Error received. For more information about error codes, see: https://goo.gl/d76uvB');
  ...
  sendTypingIndicator(recipientId, milliseconds) {
    ...
    if (milliseconds > 20000) {
      milliseconds = 20000;
      this.logger.error('sendTypingIndicator: max milliseconds value is 20000 (20 seconds)');
    }
  ...
  getUserProfile(userId) {
    const url = `${this.graphUrl}/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${this.accessToken}`;
    return fetch(url)
      .then(res => res.json())
      .catch(err => this.logger.info(`Error getting user profile: ${err}`));
  }
  ...
  conversation(recipientId, factory) {
    if (!recipientId || !factory || typeof factory !== 'function') {
      return this.logger.error(`You need to specify a recipient and a callback to start a conversation`);
    }
  ...
  _initWebhook() {
    this.app.get(this.webhook, (req, res) => {
      if (req.query['hub.mode'] === 'subscribe' && req.query['hub.verify_token'] === this.verifyToken) {
        this.logger.info('Validation Succeded.')
        res.status(200).send(req.query['hub.challenge']);
      } else {
        this.logger.error('Failed validation. Make sure the validation tokens match.');
        res.sendStatus(403);
      }
    });
    ...
    data.entry.forEach((entry) => {
      ...
      entry.messaging.forEach((event) => {
        ...
        if (event.optin) {
          ...
          this.logger.info('Webhook received unknown event: ', event);
        }
      });
    });
  }
